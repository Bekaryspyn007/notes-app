/**
 * client.js — Чачапури
 * Меню · Корзина · Заказ · Оплата (3 варианта) · Бронь · Официант
 */
...
function loadCart() {
  try { return JSON.parse(localStorage.getItem('cha_cart') || '[]'); } catch { return []; }
}
function saveCart() {
  localStorage.setItem('cha_cart', JSON.stringify(cart));
  updateBadge();
}
...
function addToCart(id) {
  ...
  saveCart();
  toast(`${dish.emoji||'✓'} ${dish.name} добавлено`, 'ok');
}
...
function renderCart() {
  ...
  const items = cart.map(c => {
    ...
    const img = d.photo_url
      ? `<img class="cart-img" src="${d.photo_url}" alt="${d.name}"/>`
    ...
    <div class="cart-item">
      ...
      <div class="dish-comment-wrap">
        <input class="dish-comment-input" type="text"
          placeholder="Без лука, extra соус..."
          value="${c.comment||''}"
  ...
  wrap.innerHTML = `
    ...
    <div class="total-row">
      <span class="total-label">Позиций</span>
      <span style="font-size:13px;color:var(--muted)">${cart.reduce((s,c)=>s+c.qty,0)} шт.</span>
    </div>
...
function renderOrderPage() {
  ...
  if (!cart.length) {
    el.innerHTML = '<div style="text-align:center;padding:18px;color:var(--muted);font-size:13px">Корзина пуста</div>';
  } else {
    const rows = cart.map(c => {
      const d = menu.find(m => m.id === c.id);
      if (!d) return '';
      const comm = c.comment
        ? `<span style="font-size:11px;color:var(--green3);margin-left:4px">(${c.comment})</span>`
        : '';
      return `<div class="order-sum-row"><span>${d.name} × ${c.qty}${comm}</span><span>${(d.price*c.qty).toLocaleString('ru')} ₸</span></div>`;
    }).join('');
    el.innerHTML = `
      <div class="order-sum-label">Состав заказа</div>
      ${rows}
      <div class="order-sum-row total">
        <span>Итого</span><span>${cartTotal().toLocaleString('ru')} ₸</span>
      </div>`;
  }
  ...
  if (sel) {
    sel.innerHTML = `<option value="">— Выберите официанта —</option>` +
      CONFIG.waiters.map(w => `<option value="${w}">${w}</option>`).join('');
  }
}
...
async function submitOrder() {
  ...
  if (!waiter)       { toast('Выберите официанта', 'err'); return; }
  ...
  try {
    ...
    console.error(err);
    toast('Ошибка при отправке. Попробуйте ещё раз.', 'err');
  } finally {
    btn.disabled = false; btn.textContent = 'Отправить заказ';
  }
}

/* ── PAYMENT PAGE — 3 варианта ── */
function showPaymentPage(total) {
  ...
  grid.innerHTML = CONFIG.payments.map(p => `
    <a href="${p.link}" target="_blank" class="pay-method-btn" style="--pay-color:${p.color}">
      <div class="pay-method-icon">${p.icon}</div>
...
function selectTime(t) {
  selTime = t;
  renderTimeSlots();
  document.getElementById('resTimeView').value = t;
}
...
async function submitReservation() {
  ...
  try {
    ...
    const dateStr = new Date(date + 'T12:00:00').toLocaleDateString('ru', {
    ...
    ['resName','resPhone','resComment','resTable'].forEach(id => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ph = document.getElementById('resPhoneLink');
